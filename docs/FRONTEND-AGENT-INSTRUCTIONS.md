# FRONTEND AGENT — FULL TASK LIST
## Read this entirely before touching any file. Do every task in order.

---

## CONTEXT

This is an AI study platform. Students go through onboarding, get a study roadmap, then click on roadmap nodes to start lessons. Lessons happen as a chat with an AI tutor. The backend already supports all of this. The frontend is partially built but has critical bugs and missing pieces.

**Your job: fix 2 files.**
- `frontend/src/pages/SubjectDetail.tsx`
- `frontend/src/pages/Tutor.tsx`

Do NOT touch any other files. Do NOT touch anything in `backend/` or `shared/`.

---

## TASK 1 — Fix SubjectDetail.tsx: node clicks must go to tutor, not /lesson

**File:** `frontend/src/pages/SubjectDetail.tsx`

### Problem
Right now when a student clicks a roadmap node, it navigates to `/lesson/:nodeId`. That page is broken and deprecated. Lessons now happen on the tutor page at `/tutor/:subject`.

### Find this code (around line 300-303):
```typescript
const onClick = () => {
  if (locked) return
...
### Also find the "Start now" button at the bottom of the file (around line 356-359):
```typescript
...
That is all for SubjectDetail.tsx.

---

## TASK 2 — Rewrite Tutor.tsx

**File:** `frontend/src/pages/Tutor.tsx`

The current Tutor.tsx has these critical problems:
1. Sends wrong `mode` value ("sprint" instead of "lesson"/"chat")
2. Does NOT send `mode`, `topic`, `nodeId` fields in the API payload — the backend needs these
3. Auto-sends wrong first message for lesson mode
4. Does NOT parse `[PHASE:xxx]` markers that Claude puts in responses
5. Has no lesson phase indicator UI
6. Has no lesson-complete state (XP banner + back to roadmap button)

**Replace the entire contents of `frontend/src/pages/Tutor.tsx` with this:**
...
## TASK 3 — Verify the route for /tutor/:subject exists in App.tsx (or wherever routes are defined)

Find the router config file (likely `frontend/src/App.tsx` or `frontend/src/main.tsx` or `frontend/src/router.tsx`).

Make sure there is a route like this:
```tsx
<Route path="/tutor/:subject" element={<Tutor />} />
```

If it already exists, do nothing. If it does not exist, add it.

Also make sure there is NO route pointing to `/lesson/:nodeId` that would conflict (or if there is one, leave it — it just won't be navigated to anymore).

---

## TASK 4 — Double-check imports in Tutor.tsx

The new Tutor.tsx uses `ReactMarkdown`. Make sure it is installed. In `frontend/`, run:
```
...
It should already be installed (it is used in SubjectDetail.tsx). If it is there, do nothing.

---

## DONE

After making these changes, the full lesson flow will work:

1. Student opens `/dashboard/Calculus%201`
2. Clicks a roadmap node → navigates to `/tutor/Calculus%201?mode=lesson&topic=Limits&nodeId=calc1-limits`
3. Tutor page auto-sends `"Start lesson on: Limits"` to backend
4. Backend Claude runs 4-phase structured lesson
5. Phase markers `[PHASE:xxx]` are stripped from display, used to update the phase indicator
6. When `[PHASE:complete]` arrives → lesson complete banner shows → student clicks "Back to roadmap"
7. Node is already marked complete in the DB (backend does this automatically)
